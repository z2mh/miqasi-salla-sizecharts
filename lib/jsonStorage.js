...
class JsonStorage {
  ...
  saveChart(merchantId, productId, chartData) {
    ...
    const isNewChart = !charts[productId];
    
    charts[productId] = {
      ...chartData,
      updated_at: timestamp,
      created_at: charts[productId]?.created_at || timestamp,
      merchant_id: merchantId,
      product_id: productId
    };

    const success = this.saveMerchantCharts(merchantId, charts);
    
    if (success) {
      const action = isNewChart ? 'CREATED' : 'UPDATED';
      console.log(`🎯 SIZE CHART ${action}:`);
      console.log(`   📅 Time: ${timestamp}`);
      console.log(`   👤 Merchant: ${merchantId}`);
      console.log(`   📦 Product: ${productId}`);
      console.log(`   📏 Sizes: ${chartData.rows?.map(r => r.size).join(', ') || 'N/A'}`);
      console.log(`   📊 Total Charts: ${Object.keys(charts).length}`);
    }
